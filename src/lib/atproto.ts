...
export async function restorePreviousSession(saved: SavedSession | null): Promise<
  | {
...
export async function getRecommendedDidCredentials(client: AtprotoClient): Promise<RecommendedDidCredentials> {
  const response = await client.agent.com.atproto.identity.getRecommendedDidCredentials()
...
export function getLatestValidAuditEntry(entries: PlcAuditEntry[]): PlcAuditEntry {
...
export async function submitDirectPlcOperation(did: string, operation: PlcSignedOperation) {
...
export function parseRawPlcOperation(value: string): PlcUnsignedOperation {
  ...
  if (!isRecord(parsed.verificationMethods)) {
    throw new Error('Raw PLC JSON must include an object "verificationMethods".')
  }
...
export async function addRotationKeyFromPasskey(params: {
  ...
  const rotationKeys = Array.from(new Set([...currentPlc.rotationKeys, params.passkeyDidKey]))

  const signed = await params.client.agent.com.atproto.identity.signPlcOperation({
    rotationKeys,
